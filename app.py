...
st.title("雲乳食品科技股份有限公司")
st.markdown("### 產線與殺菌排程效能系統 (連線版)")
...
PRODUCTS = [
    "台牧-生乳---殺菌", "台牧-A2β生乳", "雲乳-純濃鮮乳", "台牧-茶の魔手專用", 
    ...
    "英泉-全脂牛乳---殺菌", "英泉-全脂牛乳 1837", "英泉-全脂牛乳 946", "牛奶本味 946", "抹茶本位 946", "奶茶本位 946", "芝麻本位 946", "果汁牛乳 946",
    "台牧-六甲田莊巧克力牛乳乳飲品 946", "台牧-六甲田莊咖啡牛乳乳飲品 946",
    "AGV-寒天檸檬400", "AGV-寒天百香400", "AGV-寒天仙草400", "AGV-番茄蜂蜜綜合蔬菜汁400",
    "英泉-巧克力200", "英泉-麥芽210", "英泉-蘋果210", "英泉-草莓210", "英泉-優酪乳乳酸飲料", "英泉-蔓越莓乳酸飲料",
    "台牧-六甲田莊鮮乳210", "台牧-極選牛乳210", "台牧-珍稀牛乳210"
]
...
with tab_input:
    st.header("現場生產紀錄輸入")
    today = st.date_input("紀錄日期", date.today())

    equip_type = st.radio("選擇作業對象類別", ["殺菌機", "生產線"])
    ...
    if equip_type == "殺菌機":
        ...
        with col2:
            end_time = st.time_input("結束時間 (作業結束)", step=1)
        
        st.markdown("---")
        st.subheader("🎯 設備能力參數")
        capacity = STERILIZER_CAPACITY.get(selected_equip, 0)
        st.text_input("設備能力 (L/H)", value=f"{capacity} L", disabled=True)
        ...
        col1, col2 = st.columns(2)
        with col1:
            start_time = st.time_input("開始時間 (首件/作業開始)")
        with col2:
            end_time = st.time_input("結束時間 (末件/作業結束)")
        ...
        if is_production:
            ...
            with col3:
                bottle_count = st.number_input("實際生產數量 (瓶)", min_value=0, value=5000, step=100)
            with col4:
                bottle_weight = st.number_input("單瓶容量/重量 (ml/g)", min_value=0, value=946, step=1)
            
            st.markdown("---")
            st.subheader("🎯 效能分析參數")
            col5, col6 = st.columns(2)
            with col5:
                standard_rate = st.number_input("設備標準產能 (瓶/H)", min_value=1, value=6000, step=100)
            with col6:
                batch_tons = st.number_input("調配(生產)噸數 (T)", min_value=0.0, value=5.0, step=0.1)

    st.markdown("---")
    remarks = st.text_area("備註 (異常原因說明)", placeholder="若有異常、維修或特殊狀況，請簡述說明...")

    # --- 資料寫入與系統自動計算邏輯 ---
    if st.button("確認送出紀錄", type="primary"):
        if end_time <= start_time:
            st.error("錯誤：結束時間不得早於或等於開始時間！")
        elif not db_connected:
            st.error("資料庫未連線，無法寫入。")
        else:
        ...
        try:
            ...
            if unique_raw_groups:
                for group_name in unique_raw_groups:
                    ...
                    total_batch_L = line_data['調配(生產)噸數(T)'].sum() * 1000
